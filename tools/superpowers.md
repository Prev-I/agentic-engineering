...
- Skills are designed for OpenCode. Other harnesses (Claude Code, Codex) have their own skill mechanisms; Superpowers skills would need to be adapted for those contexts.
...
- **The brainstorming skill prevents premature implementation.** The most common failure mode in agent-driven development is jumping straight to code. Requiring brainstorming first in practice, structured brainstorming before implementation has reduced the frequency of premature starts and scope drift in our workflow.
- **The review skill must run on a different model family for genuine adversarial assessment.** When the same model family reviews its own work, it shares the same training biases and tends to approve. Cross-family review (e.g., implementation on one model family, review on another) catches different classes of issues.
