...
- **Built-in agents**: plan, build, explore, and scout — each optimized for a different phase of work.
...
- **Permission rules (edit/bash/task) are the primary safety mechanism for subagents.** A reviewer agent with `edit: false` and `bash: false` physically cannot modify code, regardless of what its instructions say. This is a stronger guarantee than instruction-based constraints alone.
